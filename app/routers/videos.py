...
from ..utils.uploads import build_asset_url_from_base, resolve_asset_url_path, rewrite_asset_url_base
...
async def get_video(id: str, request: Request):
    ...
    video = await ensure_video_stream(video, str(request.base_url).rstrip('/'))
    return serialize_video(video, comments, media_base_url=str(request.base_url).rstrip('/'))
